 ...
 * include the dataset of the user upload, the datasets the groups which the
 * user join upload, and the dataset of public if the user share the some of his
 * dataset
 * 
 * @author:lp
 * */
public class DataSets extends ActionSupport
{
	...
	private int total = 1;
	private final List<String> arrayuser = new ArrayList<String>();
		...
		try
		{
			...
			else
			{
				...
				for (Element memberIncludeUser : membersIncludeUser)
				{
					...
					// (List<Element>)group.getChild("member");

...
var drawSamples1 = function(filename,fieldstr, rowstr, espgCode, projString){
...
var checkSamples1 = function(filename, projInfo){
    var flag = false;
    var layers = map.layers;
	for(var i =0; i< layers.length; i++){    
		...
		ajax.onreadystatechange=function(){          
			if(ajax.readyState==4){
				if(ajax.status==200){
					...
					var samplefields=ajax.responseText.pJSON().samplefields;           
					var samplerows=ajax.responseText.pJSON().samplerows;
				    drawSamples1(filename,samplefields,samplerows, espgCode, newProjString);                     
				}
			 }    
		};
		ajax.send(null);      
